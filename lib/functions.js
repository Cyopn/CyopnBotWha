...
const normalizeStickerVideo = async (buffer) => {
	...
	const outputPath = path.join(os.tmpdir(), `cyopn-sticker-out-${Date.now()}-${suffix}.webp`);
	try {
		...
		const ffmpegArgs = [
			...
			"-an",
			"-t", "6",
			"-vf", "scale=512:512:force_original_aspect_ratio=decrease,format=rgba,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=0x00000000,fps=15,format=yuva420p",
			"-c:v", "libwebp",
			"-lossless", "0",
			"-q:v", "60",
			"-preset", "default",
