...
const getAfk=async (client, message, tipo, args)=>{
    ...
    if (tipo==="tag"){
        ...
        if(afkdB.has(groupId) && afkdB.has(`${groupId}.${userCheck}`)){
            ...
            if(status==="afk"){
                await client.reply(from, `El usuario que estas mecionando no esta disponible ahora \n_Razon: ${reason}_ \n_Tiempo restante: ~${(delay/60).toFixed(2)} Minuto(s)_`, id)
            }
        }
    }
}

module.exports = { getBuffer, createSt, getRed, uploadImage, loadCommands, getAfk }