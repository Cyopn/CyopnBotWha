...
module.exports.run = async (client, message, args, config) => {
  ...
  if (!args.join("")) {
    ...
    if (isUrl) {
      ...
      if (res.includes("Error establishing a database connection"))
        return client.reply(
          from,
...
Intenta mas tarde`,
      ...
      if (res.medias === undefined)
        return client.reply(from, `El enlace no es valido`, id);
      if (res.medias[1] != undefined) {
        await client
          .sendFile(from, res.medias[1].url, "nose", `w`, id)
          .catch((e) => {
            if (
              e
                .toString()
                .includes("Error: Evaluation failed: Error: MediaFileTooLarge:")
            ) {
              client
...
Hora: ${new Date().getHours()}:${new Date().getMinutes()}:${new Date().getSeconds()}`,
        ...
        res.medias[1] === undefined &&
        res.medias[0].url != undefined
      ) {
        await client
          .sendFile(from, res.medias[0].url, "nose", `w`, id)
          .catch((e) => {
