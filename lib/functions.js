...
const checkAfk=async (client, message)=>{
    const { from, sender, author, isGroupMsg, chat, body, quotedMsg, mentionedJidList } = message
    if(!isGroupMsg) return
    const userCheck=quotedMsg?quotedMsg.author.replace('@c.us', ''):mentionedJidList && mentionedJidList[0]?mentionedJidList[0].replace('@c.us', ''):author.replace('@c.us', '')
    
}

module.exports = { getBuffer, createSt, getRed, uploadImage, loadCommands, checkAfk }