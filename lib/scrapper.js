...
const tiktok = async (url) => {
    ...
    try {
        ...
        if (result.status === "success") {
            ...
            return { data: { error: "No se encontró contenido." } };
        }
    } catch (e) {
    }
    return { data: results };
...
async function facebook(url) {
    ...
    form.append("url", url);
    form.append("siteurl", "https://instatiktok.com/");
    form.append("action", "https://instatiktok.com/api");
    form.append("platform", "instagram");
    const res = await fetch("https://instatiktok.com/api", {
        method: "POST",
        body: form,
        headers: {
            origin: "https://instatiktok.com",
            referer: "https://instatiktok.com/",
            "user-agent":
                "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
        },
    });
    if (!res.ok) return { status: "error", data: { msg: "server error" } };
    const raw = await res.text();
    let html = raw;
    try {
        const json = JSON.parse(raw);
        html = json?.html || json?.data || raw;
    } catch (error) {
        html = raw;
    }
    links = [];
...
async function ytmp4(url) {
    try {
        ...
        const res = await ytdlp.downloadVideo(url, "highest", { output: `./temp/${tempname}.%(ext)s` });
        ...
        return { filePath: mp4Path, title: res.info[0].fulltitle };
    } catch (e) {
        throw e;
    }
}
...
async function ytmp3(url) {
    try {
        ...
        const res = await ytdlp.downloadAudio(url, "mp3", { output: `./temp/${tempname}.%(ext)s`, audioQuality: "0" });
        const downloadedPath = res.filePaths[0];
        const mp3Path = downloadedPath.replace(/\.[^/.]+$/, ".mp3");
        if (downloadedPath !== mp3Path) {
            execSync(`ffmpeg -y -i "${downloadedPath}" -vn -c:a libmp3lame -q:a 0 "${mp3Path}"`);
            if (fs.existsSync(downloadedPath)) {
                fs.unlinkSync(downloadedPath);
            }
        }
        return { filePath: mp3Path, title: res.info[0].fulltitle };
    } catch (e) {
        throw e;
    }
}

async function cleanTemp(file, delay = 0) {
    if (delay > 0) {
        setTimeout(() => {
            unlink(file).catch(e => console.error(e));
        }, delay);
