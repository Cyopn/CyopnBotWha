...
async function ytmp3(url) {
    try {
        ...
        const res = await ytdlp.downloadVideo(url, "bestaudio[ext=m4a]/bestaudio/best", { output: `./temp/${tempname}.%(ext)s` });
        const originalPath = res.filePaths[0];
        const mp3Path = originalPath.replace(/\.[^/.]+$/, ".mp3");
        execSync(`ffmpeg -y -i "${originalPath}" -vn -c:a libmp3lame -q:a 2 "${mp3Path}"`);
