...
const lvlFunc = async (client, message) => {
    ...
    if (lvldB.has(groupId) && lvldB.has(`${groupId}.${sid}`)) {
        ...
        let b = body.split(" ")
        console.log(b)
        const sxp = ((Math.random() * b.length / 2)).toFixed(0) + 1
        let lvlup = 5 * (level ** 2) + 50 * level + 100
        console.log(`${groupId} - ${sid} - ${sxp} - ${lvlup - xp - sxp}`)
        ...
        if ((xp + sxp) >= lvlup) {
            ...
            await client.sendTextWithMentions(from, `Felicidades @${sender.id} has avanzado de nivel: ${a.level} \nSigue asi!`)
        } else {
...
const youtubeDl = async (args) => {
    const res = await axios.get(`https://api-fgmods.ddns.net/api/ytmp3?url=${args[0]}&apikey=${args[1]}`)
...
module.exports = { getBuffer, createSt, getRed, uploadImage, loadCommands, getAfk, fancyTime, welcome, lvlFunc, emojiFix, youtubeDl }