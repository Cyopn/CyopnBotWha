...
const evalLevel = async (sock, msg, content) => {
    try {
        ...
        if (!String(remoteJid || "").endsWith("@g.us") || !participant) return;
        const gid = remoteJid.split("@")[0];
        const gci = await dbg.get(`${gid}.rank`);
        if (!gci) return;
        const uid = participant.split("@")[0]
        const sxp =
...
const getGroupSettings = async (gid) => {
    const id = String(gid || "").split("@")[0];
    return id ? ((await dbg.get(id)) || {}) : {};
};

const getAllGroupSettings = async () => {
    const settings = {};
    for (const gid of await dbg.keys()) settings[gid] = (await dbg.get(gid)) || {};
    return settings;
};

const setGroupSetting = async (gid, key, value) => {
    const id = String(gid || "").split("@")[0];
    if (!id || !String(key || "").trim()) return false;
    await dbg.set(`${id}.${key}`, value);
    return true;
};
