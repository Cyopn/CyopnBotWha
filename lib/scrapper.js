...
async function ytmp4(url) {
    try {
        ...
        execSync(`ffmpeg -i "${res.filePaths[0]}" -c:v copy -c:a aac -strict experimental "${res.filePaths[0].replace(".mkv", ".mp4")}"`);
        return { filePath: res.filePaths[0].replace(".mkv", ".mp4"), title: res.info[0].fulltitle };
    } catch (e) {
        throw e;
    }
}

async function cleanTemp(file) {
    try {
        fs.unlinkSync(file);
    } catch (e) {
        console.error(e);
