...
let dba = new db.MegaDBFull("afk", {
	dir: "./"
});

let dbe = new db.MegaDBFull("error", {
	dir: "./"
});
...
const errorHandler = async (sock, msg, command, err) => {
	try {
		...
		dbe.set(`${command}.${new Date().toLocaleDateString().replaceAll("/", "-")}T${new Date().toLocaleTimeString().replaceAll(":", "-").replaceAll(".", "-")}`, {
			error: err.toString(),
			errorobj: err,
			message: msg,
		});
